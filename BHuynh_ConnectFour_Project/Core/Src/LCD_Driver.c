...
void visualDemo(void)
{
	...
	HAL_Delay(1500);
	LCD_Clear(0, LCD_COLOR_WHITE);
	LCD_Draw_Vertical_Line(50,50,250,LCD_COLOR_MAGENTA);
	HAL_Delay(1500);
...
void LCD_DrawString(uint16_t Xpos, uint16_t Ypos, char string[]) {

	uint16_t i = 0;

	while (string[i] != '\0') {
		LCD_DisplayChar(Xpos, Ypos, (uint8_t)string[i]);
	    Xpos += 15;
	    i++;
	}
...
/*void EnableTouchInterrupt() {
	...
	STMPE811_Write(STMPE811_INT_CTRL, 0x1);
	STMPE811_Write(STMPE811_INT_EN, 0x1);
